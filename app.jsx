...
function App() {
  ...
  const handlePlace = () => {
    setScreen('confirm');
    setTimeout(() => setShowGift(true), 2600);
  };
  ...
  else if (screen === 'checkout') view = <Checkout cart={cart} total={confirmedTotal} pickup={pickup} mode={mode} store={STORE} onPlace={handlePlace} onBack={() => setScreen('cart')}/>;
  else if (screen === 'confirm') view = <Confirmation store={STORE} pickup={pickup} mode={mode} code="BT-4829" total={confirmedTotal} saved={saved} onScratch={() => { setShowGift(false); setScreen('scratch'); }} onDone={() => { setCart([]); setScreen('home'); }}/>;
  else if (screen === 'scratch') view = <ScratchCard onDone={() => { setCart([]); setScreen('home'); }}/>;
  else if (screen === 'passport') view = <Passport user={USER} cartCount={cartCount} onBack={() => setScreen('home')} onTab={goTab}/>;

  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '100vh', padding: 20 }}>
      <Phone>
        <div key={screen} className="screen-anim" style={{ height: '100%' }}>{view}</div>
        {showGift && <SurpriseGiftModal onScratch={() => { setShowGift(false); setScreen('scratch'); }} onSkip={() => setShowGift(false)}/>}
      </Phone>
    </div>
  );
}
